...
import { Loader2, User, Eye, EyeOff, MessageCircle, Fingerprint, Sparkles, CheckCircle2, ShieldAlert } from "lucide-react";
...
function SuccessOverlay({ name }: { name: string }) {
  return (
    <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-gradient-to-br from-blue-600 via-indigo-500 to-violet-600 anim-success-bg">
      ...
      <div className="relative flex items-center justify-center mb-8">
        <div className="absolute w-36 h-36 rounded-full bg-blue-300 opacity-20 anim-success-ring" />
        <div className="absolute w-28 h-28 rounded-full bg-blue-300 opacity-15"
          style={{ animation: "successRing 1s ease-out 0.3s infinite" }} />
        <div className="w-24 h-24 rounded-full bg-gradient-to-br from-blue-400 to-indigo-600 flex items-center justify-center shadow-2xl anim-success-pop">
          <CheckCircle2 size={40} className="text-white drop-shadow-lg" />
        </div>
      </div>
      <div className="text-center px-8 anim-success-text">
        <p className="text-white/70 text-xs font-semibold mb-2 uppercase tracking-[0.2em]">Login Successful</p>
        ...
        <div className="mt-8 w-56 mx-auto h-1.5 bg-white/20 rounded-full overflow-hidden">
          <div className="h-full bg-white/90 rounded-full" style={{ animation: "loginFadeIn 1.5s ease forwards", width: "100%" }} />
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/40 flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-10 gspp-gradient gradient-animate px-6 py-12 text-center text-white overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="text-white/75 text-sm font-medium">Employee authentication portal</p>
        </div>
      </div>

      {/* Login Card */}
      <div className="relative z-10 flex-1 px-4 py-6 max-w-md mx-auto w-full">
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/50 shadow-xl shadow-blue-500/5 p-6 anim-fade-in">
          ...
          <form onSubmit={handleWorkerLogin} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-3.5 rounded-xl border-2 border-gray-100 text-sm font-medium focus:outline-none focus:ring-0 focus:border-blue-400 bg-gray-50/50 hover:bg-white transition-all placeholder:text-gray-300"
                  autoFocus
                />
            ...
            {error === "NEEDS_PERMISSION_UPDATE" && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 anim-fade-in">
                <p className="text-sm font-semibold text-amber-800 mb-1">Account Not Yet Configured</p>
                <p className="text-xs text-amber-700 mb-3">Your account permissions have not been set up yet. Please contact your Administrator to update your account before logging in.</p>
                <a
                  href={`https://wa.me/${import.meta.env.VITE_ADMIN_WHATSAPP?.replace(/\D/g, '')}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 w-full py-2 rounded-lg bg-amber-600 hover:bg-amber-700 text-white text-xs font-semibold transition-all"
                >
            ...
            <button
              ...
              className="w-full py-3.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold text-sm hover:from-blue-700 hover:to-indigo-700 active:scale-[0.98] transition-all disabled:opacity-60 flex items-center justify-center gap-2 shadow-lg shadow-blue-500/25 border border-blue-500/20"
            >
          ...
          <div className="mt-5 rounded-xl border border-green-100 bg-gradient-to-br from-green-50/80 to-emerald-50/80 p-4">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 active:scale-[0.98] text-white text-sm font-semibold transition-all shadow-md shadow-green-500/20"
            >
