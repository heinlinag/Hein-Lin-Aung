  ...
  workers: router({
    ...
    verify: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (!worker) throw new TRPCError({ code: "NOT_FOUND", message: "Employee ID not found" });
        const perms = worker.permissions ? JSON.parse(worker.permissions) : null;
        return { id: worker.id, workerID: worker.workerID, name: worker.name, department: worker.department, userLevel: worker.userLevel, permissions: perms };
      }),
    ...
    add: publicProcedure
      .input(z.object({
        ...
        permissions: z.object({
          submitOrder: z.boolean().default(false),
          viewStock: z.boolean().default(false),
          nprmModifyOrder: z.boolean().default(false),
          customerSample: z.boolean().default(false),
          qrScanner: z.boolean().default(false),
          viewChat: z.boolean().default(false),
          viewNotifications: z.boolean().default(false),
        }).optional(),
