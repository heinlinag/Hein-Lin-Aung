...
## Permission-Based Access Control System (Jul 23)
- [x] Schema: add `permissions` JSON column to workers table (replaces level-based access)
- [x] Schema: define permission keys: submitOrder, viewStock, nprmModifyOrder, customerSample, qrScanner, viewChat, viewNotifications
- [x] DB migration: apply schema changes
- [x] Server: update getWorker/upsertWorker to include permissions field
- [x] Server: add admin procedure to update worker permissions
- [x] Server: add hasPermissions check (null = not yet configured = show contact admin)
- [x] Admin Panel: redesign user add/edit form with permission checkboxes instead of level dropdown
- [x] Admin Panel: show permission badges on user list cards
- [x] Frontend: update auth context to expose permissions from worker session
- [x] Frontend: update route guards to check permissions instead of level
- [x] Frontend: hide nav items user doesn't have permission for
- [x] Frontend: login page — if worker has no permissions set, show "Please contact Administrator to update your account" message
