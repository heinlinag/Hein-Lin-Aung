...
## NPRM Urgent Feature (Jun 5)
- [x] Add isUrgent column to pendingRequests table
- [x] Server: toggleUrgent procedure — Level 1 can toggle on their own pending requests
- [x] UI: "Arrange it Urgent" button in Update Info dropdown
- [x] UI: Red "Order is Urgent" badge on NPRM card when isUrgent=true and pending
- [x] Badge disappears when Level 1.1/2 marks as in-process)
- [ ] Schema: Add `isUrgent` boolean column to NPRM Modify Order table (nprm_modify_orders)
- [ ] Generate and apply migration SQL
- [ ] Server: Add procedure to toggle isUrgent flag on NPRM order
- [ ] UI: Add "Arrange it Urgent" button/checkbox in Update Info modal (ApprovalCenter)
- [ ] UI: Show red "Order is Urgent" badge on NPRM card when isUrgent=true
- [ ] UI: Hide urgent badge when Level 1.1/2 starts processing (status changes to in-process)
